...
sudo rm -rf /tmp/nogfid{1,2}/.landfill
diff <($AREQUAL /tmp/nogfid1) <($AREQUAL /tmp/nogfid2)
assert_success $?
}
...
$GLUSTERFSDIR/gluster volume create hosdu $(hostname):/tmp/nogfid1/
...
sleep 10
$GLUSTERFSDIR/gluster volume replace-brick hosdu $(hostname):/tmp/nogfid1/ $(hostname):/tmp/nogfid2/ start | grep "replace-brick started successfully" 2>/dev/null 1>/dev/null
if [ $? -ne 0 ];then
...
do
	...
	$GLUSTERFSDIR/gluster volume replace-brick hosdu $(hostname):/tmp/nogfid1/ $(hostname):/tmp/nogfid2/ status >/tmp/tempfile # 2>/dev/null 1>/dev/null
	if [ $? -ne 0 ];then
		temp=2
	fi
...
if [ $temp -eq 1 ];then
	...
	$GLUSTERFSDIR/gluster volume replace-brick hosdu $(hostname):/tmp/nogfid1/ $(hostname):/tmp/nogfid2/ commit;
        graceful_cleanup ;
elif [ $temp -eq 2 ];then
